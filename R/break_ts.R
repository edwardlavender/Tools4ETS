...
break_ts <-
  function(dat,
    ...
    #### Define a list, one element for each unique time series
    if(is.null(ind)){
      dat_ls <- list(dat)
    } else{
    ...
    #### Define positions at which to break the time series, if these have not been provided
    if(length(pos) == 0){
      pos <-
        # Loop over the list of time series and the positions provided in n
        ...
        mapply(dat_ls, n, FUN = function(dind, nsel){
          # sample nsel positions from each time series
          nn <- nrow(dind)
    ...
    #### Define a list, with one element for each time series, defining where we'll break each time series
    dat_ls_breaks <-
      # Loop over each unique time series, the pos list, and the before/after functions
      mapply(dat_ls, pos, before, after, FUN = function(dind, psel, bf, af){
        ...
        dind$pbreak <- FALSE
        # If there are positions at which we'll break the time series
        if(length(psel) > 0){
          # For each position
          for(p in psel){
        ...
        # Return our vector of TRUE/FALSE for each time series
        return(dind$pbreak)
      }, SIMPLIFY = FALSE)

    #### Define the positions at which the time series will be broken
    pbreaks_logic <- as.vector(unlist(dat_ls_breaks))
    pbreaks <- which(pbreaks_logic)

    #### Break the time series
