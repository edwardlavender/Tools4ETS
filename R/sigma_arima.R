#' @title Compute the standard deviation of white noise for an AR1 process with a desired AR1 parameter and standard deviation
#' @description In statistics, it is often instructive to simulate observations according to a known data-generating process and then model simulated observations in different ways, to determine the extent to which simulated observations are recovered under different scenarios. To do this for a simple Gaussian model, the user might specify a formula for the linear predictor and a sigma parameter (i.e. the standard deviation among residuals) and then simulate observations from this model. However, the simulation of autocorrelated residuals with these desired properties is slightly more complex. This is because a specified sigma parameter in \code{\link[stats]{arima.sim}} is not the same as the standard deviation of the resultant AR1 process (see Example code, below). In brief, this is because the standard deviation of an AR1 process is affected by both the deterministic and stochastic components of an AR1 process, not just the stochastic process as is the case for models without autocorrelation. \code{sigma_arima()} is a function which allows the user calculate the necessary standard deviation of the stochastic process, given the extent of autocorrelation and the desired sigma value (i.e. the standard deviation of residuals). This is useful for (a) simulating autocorrelated observations with known parameters, either from a user-specified data-generating process or from a model (i.e. posterior simulation, of the kind implemented by \code{\link[Tools4ETS]{simulate_posterior_obs}}).
#'
#' @param AR1 A numeric input which defines the AR1 parameter. This may be simulated or estimated.
#' @param sigma A numeric input which defines the desired standard deviation of the AR1 process. This could be a residual standard deviation (also misnomed also 'residual standard error', e.g., in \code{\link[stats]{summary.lm}}'s output, from a fitted model), see \code{\link[stats]{sigma}}. This may be simulated or estimated.
#'
#' @returns The function returns a number which defines the SD parameter of the stochastic part of an AR1 process necessary to reproduce the desired residual standard deviation (\code{sigma}).
#'
#' @examples
#'
#' #### Simulate some observations
#' # We'll simulate a reasonably large number of observations to generate accurate estimates.
...
#' d$mu <- 0.001 * d$x^2
#'
#' #### The SD of an AR1 process is different from the SD of the outcome of the process
...
#' # We can see that the SD of the repsonse is different from the simulated SD
...
#' # Note that this code is set up to use multiple compute cores and may take a few minutes...
...
#' }
#'
#' #### Use (2): (B) Compare simulated and infered residual standard deviation
#' # The desired sigma represents the residual standard deviation ('residuals SE'),
#' # ... (i.e., the variation around a model's expectedc values). Thus, sigma_arima()
...
#' # To show this, let's implement a bam() model with the correct AR1 parameter value
...
#'
#'
#' #### Use (2): Compute appropriate sigma from model for posterior simulation
#' # ... of prediction intervals, accounting for AR1 process.
