#' @title Summarise a timeseries in bins
#' @description This function averages a timeseries (i.e., all of the values of a response variable collected through time) in each of a series of user-specified bins. The function can handle independent timeseries (e.g., for multiple individuals), average numeric explanatory variables alongside the response, incorporate other 'static' variables (e.g., the sex of each individual) and implement different types of averages.
#' @param dat A dataframe which includes observations collected over time.
#' @param split (optional) A character that defines the name of the column in \code{dat} that distinguishes independent timeseries.
...
#' @param breaks A number or vector which defines the number of unique cut points or the unique cut points themselves at which each independent timeseries is cut into bins. This is passed to \code{\link[base]{cut}}.
...
#' @param first (optional) A character vector that defines the name(s) of the column(s) in \code{dat} for which the first observation for each independent timeseries should be retained in the averaged dataframe. This is useful for 'static' variables which do not change through time (e.g., the sex of each individual).
...
average_ts <- function(dat,
  ...
  #### Split timeseries into independent sections
  if(!is.null(split)){
    if(verbose) cat("Step 1: Splitting timeseries into independent segments...\n")
    dat_ls <- split(dat, dat[, split])
  } else{
    dat_ls <- list(dat_ls)
  }

  #### Average each independent timeseries
  ## Loop over each independent timeseries...
  if(verbose) cat("Step 2: Averaging each independent timeseries...\n")
  dat_ls <- pbapply::pblapply(dat_ls, function(dat){

    ## Cut timeseries into breaks
    # if(verbose) cat(".. Step A: Cutting timeseries into breaks...\n")
  ...
  #### Join all independent timeseries
  if(verbose) cat("Step 3: Joining all independent timeseries...\n")
