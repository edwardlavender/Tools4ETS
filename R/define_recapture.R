...
#' @param define_time_before A function which takes a single argument ('recapture_time', in POSIXct format) and outputs a time (in POSIXct format) which defines the lower bound of a recapture window. The default is a function which returns a time 2 hours before a given recapture window.
...
#' @param before4plot A numeric input defining the number of seconds before the recapture time, estimated using the first method inputted, which is used to define the lower x limit of the plot. The default is 3600 (i.e. the timeseries plot will begin from one hour before the estimated recapture time).
#' @param after4plot A numeric input, as above, used to define the upper x limit of the plot. The default is 3600 (i.e. the timeseries plot will extend until one hour after the estimated recapture time).
#' @param ylim A numeric vector of two which specifies the y limits of the plot. Note that depth is negated internally within the function, so \code{ylim} needs to be negated too. The default is \code{ylim = c(-250, 0)}.
#' @param vcols A vector of colours. The recapture time estimated by each method is added to the timeseries plot as a vertical line, with the specified colour.
...
#'
#' @return A list with the following elements: (1) \code{recapture_event_details} (a list); (2) \code{recapture_times} (a dataframe); and, if the user has selected to remove recaptures, (3) \code{recapture rows} (a numeric vector) and (4) \code{data_depth_rem_recap} (a dataframe). (1) \code{recapture_event_details} is a list which contains an element for each recapture event. Each element IS itself a list which contains: (a) \code{data_recapture_event}, the data row from data_recaptures which describes the ID/timestamp of the event in question; (b) \code{rc_time_ls}, a list defining the estimated time of recapture by each method; (c) \code{rc_time}, a list containing the chosen estimated time of recapture, the method used (1, 2, 3 or manual) and, if the manual method is used, the click number which was used to define the time of recapture); (d) \code{reasonable_bounds}, a list containing the time window within which recapture events are considered plausible and outside of which a warning is flagged; (e) \code{recapture rows}, a numeric vector which defines all the positions in \code{data_depth} within the user-defined recapture window around the chosen estimated time of recapture for the individual in question. If no data is associated with a recapture event, a warning is given and other elements are NA/NULL. (2) \code{recapture_times} is a dataframe which, for each row in \code{data_recapture} (i.e., individual and date of recapture) specifies the timestamp of each recapture event and the method used (i.e., this is a synthesis of the information provided by each element in \code{recapture_event_details}). (3) \code{recapture_rows_all} is a numeric vector which specifies all the positions in \code{data_depth} which lie within recapture windows. (4) \code{data_depth_rem_recap} is the \code{data_depth} dataframe with these positions removed.
#'
#' @examples
#'
#' #### Method
    ...
    out <- pbapply::pblapply(data_recapture_ls, cl = NULL, function(recap){
      ...
      if(length(pos_data_depth) > 0){
        ...
        if(inspect_plot){
          ...
          ###################################################
          #### Manually identify the desired timestamps of desired moments of recapture
