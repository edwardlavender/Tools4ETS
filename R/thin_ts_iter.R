...
#' @param thin_ts_iter_ls A named list of outputs from a previous implementation of \code{thin_ts_iter()} which can be used to produce a plot. This bypasses the need to re-run the algorithm to produce plots.
#' @param p1_pretty_axis_args A named list of arguments passed to \code{\link[plot.pretty]{pretty_axis}} to make the x and y axes. These pertain to the thinning index and the autocorrelation parameter. The default options usually result in pretty axes.
#' @param p2_pretty_axis_args A named list of arguments passed to \code{\link[plot.pretty]{pretty_axis}} to make the second y axis. This pertains to the volume of data left after thinning. The default options usually result in a pretty axis.
...
#' @param add_model_predictions_args A list of arguments passed to \code{\link[plot.pretty]{add_model_predictions}} to customise the 95 percent confidence envelope for an AR1 parameter in white noise.
