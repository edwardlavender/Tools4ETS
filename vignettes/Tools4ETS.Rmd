...
3. To facilitate exploration of the consequences of timeseries structure, post-processing and model structure for model-based ecological inferences, via functions which enable the comparison of simulated timeseries and relationships with those inferred by GAMs and an interactive application.
...
The `prettyGraphics` package was initially designed as a complement to `Tools4ETS` to facilitate timeseries visualisation. In particular, the `pretty_ts()` function makes the exploration of ecological timeseries straightforward. This function is based on multiple 'building block' functions within the `prettyGraphics` package with which it is helpful to be to be familiar (see the vignette for `prettyGraphics`). In brief, `pretty_ts()` includes the following functionality:
...
Here, I cut to the chase and create a collection of plots to demonstrate the power of `pretty_ts()` for the exploration of ecological timeseries. 
...
One of the best ways to evaluate the performance of a model is to overlay observed and expected timeseries. `prettyGraphics::add_model_predictions()` makes this easy, and this is incorporated into `pretty_ts()`:
...
#### Plot observed and expected timeseries for individual "A" using pretty_ts()
...
PI <- simulate_posterior_obs(model = mod_dfp_thin1,
                             ...
                             n = 1000,
                             return = "summary")
# Adjust PI list so that it can be plotted with add_model_predictions() via pretty_ts()
names(PI)[1] <- "fit"
...
The final step is to specify a function which is used to simulate values of a response for each independent section of the linear predictor. We pass this to `sim_ts()` which uses `alpha`, `alpha_sigma_random` and `compute_lp` to compute the linear predictor and then `sim_obs` to simulate observations from the linear predictor, accounting for independent timeseries (`lpi`, identified by the `individual` column in `dat1` here, since we know there are no gaps in the timeseries). 
...
### Visualise simulated timeseries: `pretty_ts()`

We can now visualise simulated timeseries with `pretty_ts()`:
...
positions using `break_ts()`. I model processed timeseries and compare the distribution of differences between the simulated and estimated AR1 parameters among scenarios.
...
cs1_ls <-
  pbapply::pblapply(scenario, cl = cl, function(scen){
    ...
    # Optionally break the timeseries at n break points a given amount 
    if(scen > 0){
...
mapply(depth_ls, names(depth_ls), FUN = function(depth, driver){
  pretty_ts(dat1_cs3$timestamp, depth,
          add_lines_args = list(lwd = 0.5),
