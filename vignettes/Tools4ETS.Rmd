...
prettyGraphics::pretty_plot(dA$timestamp,
                         dA$dn,
                         pretty_axis_args = pretty_axis_args,
                         type = "l"
                         ) %>% invisible()
...
prettyGraphics::pretty_residuals(stats::resid(m1_dfp),
                            plot = 7,
...
prettyGraphics::pretty_residuals(stats::resid(m1_dfp_thin),
                            plot = 7,
                            pretty_axis_args = paa_acf)
```
...
prettyGraphics::pretty_residuals(residuals = mod_dfp_thin1$std.rsd,
...
sun_angle_av <-
  prettyGraphics::summarise_in_bins(x = dat4summarise_in_bins[[1]]$timestamp,
...
pbapply::pblapply(1:nsim, cl = cl, FUN = function(i){
  ...
  dat1_cs2_thin_swa_ls <-
    lapply(dat4summarise_in_bins, function(df){
      d <- prettyGraphics::summarise_in_bins(x = df$timestamp,
                                     y = df$depth,
                                     bin = "10 mins",
                                     shift = TRUE,
                                     to_plot = FALSE,
...
prettyGraphics::pretty_plot(dat1_cs3$julian_day, dat1_cs3$photoperiod,
                         xlab = "Time (Julian day)",
                         ylab = "Photoperiod (hours)",
                         type = "l")
prettyGraphics::pretty_plot(dat1_cs3$julian_day, dat1_cs3$temp,
                         xlab = "Time (Julian day)",
...
## Photoperiod
prettyGraphics::pretty_plot(dat1_cs3$photoperiod[!duplicated(dat1_cs3$julian_day)],
                         julian_day_smooth$y,
...
## Temperature
prettyGraphics::pretty_plot(dat1_cs3$temp[!duplicated(dat1_cs3$julian_day)],
                         julian_day_smooth$y,
...
mapply(depth_ls, names(depth_ls), FUN = function(depth, driver){
  pretty_ts(dat1_cs3$timestamp, depth,
          ...
          return_list = FALSE)
  prettyGraphics::pretty_plot(dat1_cs3[, driver], depth,
                           cex = 0.5,
                           col = "grey",
                           xlab = driver,
                           ylab = "Depth")
